 ...
 * @WebFilter("/*")
 * 
 * @Log4j public class forwardloginfilter implements Filter {
 * 
 ...
 * 
 * //로그인이 된 상태인지 유무 확인 HttpSession session = request.getSession(true);
 * //true(기본) : 세션객체 없으면 새로 생성 //false : 세션객체 없으면 null
 * 
 * if(session.getAttribute("authdto")==null) //no login {
 * if(request.getRequestURI().contains("/resources")) { chain.doFilter(req,
 * resp); return ; } if(request.getRequestURI().contains("/member/join")) {
 * req.getRequestDispatcher("/member/join").forward(request, response); return ;
 * }
 * 
 * String
 * msg="<i class='bi bi-exclamation-triangle' style='color:orange;font-size:1rem'></i> 로그인이 필요한 페이지 입니다."
 * ; req.getRequestDispatcher("/login?msg=" + msg).forward(request, response);
 * return ; } else //로그인 한 상태 {
 * 
 * }
 * 
 * //로그인한 상태 chain.doFilter(req, resp); }
 * 
